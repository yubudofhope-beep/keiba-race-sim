...
(function(){{
  ...
  // 馬房幅(world)。コース幅を頭数で割るが、馬マーカー径(0.7)より極端に細くしない。
  function gateCellW(){{
    return Math.max((thick*0.92*SCALE3D)/horses.length, 0.5);
  }}
  ...
  function place2(progress){{
    ...
    horses.forEach(function(h,i){{
      ...
      var laneW = Math.min(thick*0.11, (thick*0.74)/Math.max(1,horses.length-1));
      // ★馬番の並び。この垂直方向(nx2,ny2)は正が内ラチ側を向くため、
      //   (i-mid) をそのまま使うと1番が外・大きい馬番が内という逆順になっていた。
      //   1番=最内になるよう符号を反転する。
      var spread = (mid-i)*laneW*(stretch>0 ? 1.3 : 1.0);
